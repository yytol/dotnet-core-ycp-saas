...
app.ready(function (obj) {
    ...
    //加载授权列表
    pg.loadAosAuthorizeList = function (uid) {
        ...
        $.post(ycc.entityConfig.UrlEntrance + "/Api/AosUserApps/GetUserAuthList", {
            sid: ycc.entityConfig.SessionID,
            uid: uid
        }, function (e) {
            var jttp = dpz.data.json.parse(e);
    ...
    pg.loadList = function (uid, aid) {
        ...
        //设置选择状态
        for (i = 0; i < obj.data.catalogs.length; i++) {
        ...
        $.post(ycc.entityConfig.UrlEntrance + "/Api/AosUserApps/GetList", {
            sid: ycc.entityConfig.SessionID,
            uid: uid,
            aid: aid
        }, function (e) {
    ...
    obj.methods = {
        ...
        onChangeUser: function (e, ob) {
            pg.userId = ob.ID;
            pg.loadAosAuthorizeList(pg.userId);
            obj.data.list = [];
        },
        onCheck: function (e, ob) {
            $.post(ycc.entityConfig.UrlEntrance + "/Api/AosUserApps/Save", {
                sid: ycc.entityConfig.SessionID,
                UserID: pg.userId,
                AuthID: pg.authId,
                AppID: ob.ID
