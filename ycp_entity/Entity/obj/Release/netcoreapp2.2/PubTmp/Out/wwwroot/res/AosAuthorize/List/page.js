...
app.ready(function (obj) {
    ...
    obj.methods = {
        onAppSetup: function (e, ob) {
            desktop.body.show(ycc.entityConfig.UrlEntrance, "/res/AosAuthorize/AppSetup/config.json", ob);
        }
    };

    obj.methods.onAdd = function (e) {
        //desktop.body.show(ycc.entityConfig.UrlEntrance, "/res/Authorization/Add/config.json");
        desktop.dialog.show(ycc.entityConfig.UrlEntrance, "/res/AosAuthorize/Add/config.json", {
            hasHeader: true,
            width: 480,
            height: 340,
            title: "添加授权",
    ...
    obj.methods.onEdit = function (e, ob) {
        //desktop.body.show(ycc.entityConfig.UrlEntrance, "/res/Authorization/Add/config.json");
        //console.log(e);
        desktop.dialog.show(ycc.entityConfig.UrlEntrance, "/res/AosAuthorize/Edit/config.json", {
            hasHeader: true,
            width: 480,
            height: 340,
            title: "修改授权 - [" + ob.Name + "]",
    ...
    obj.methods.onDelete = function (e, ob) {
        ...
        if (confirm("确定要删除项目 \"" + ob.Name + "\" 吗?")) {
            $.post(ycc.entityConfig.UrlEntrance + "/Api/Auth/Delete", {
                sid: ycc.entityConfig.SessionID,
                ID: ob.ID
            }, function (re) {
